...
// getPortFromUser prompts user for port input
func getPortFromUser(defaultPort int) int {
	reader := bufio.NewReader(os.Stdin)
	fmt.Printf("enter server port (default: %d): ", defaultPort)
	
	input, err := reader.ReadString('\n')
	if err != nil {
		log.Printf("error reading input: %v, using default port %d", err, defaultPort)
		return defaultPort
	}
	...
	if err != nil {
		log.Printf("invalid port number: %v, using default port %d", err, defaultPort)
		return defaultPort
	}
	
	if port < 1 || port > 65535 {
		log.Printf("port %d out of range (1-65535), using default port %d", port, defaultPort)
		return defaultPort
	}
...
func main() {
	defaultPort := 20202
	fmt.Printf("enter server port (default: %d): ", defaultPort)
	port := getPortFromUser(defaultPort)
	...
	// override port from config with user input
	grpcCfg.Listener.Port = port
