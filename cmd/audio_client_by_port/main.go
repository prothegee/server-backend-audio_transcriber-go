...
// getServerAddressFromUser prompts user for server address and port
func getServerAddressFromUser(defaultHost string, defaultPort int) (string, int) {
	reader := bufio.NewReader(os.Stdin)
	
	fmt.Printf("enter server host (default: %s): ", defaultHost)
	...
	if portInput == "" {
		...
		if err != nil || p < 1 || p > 65535 {
			log.Printf("invalid port: %s, using default %d", portInput, defaultPort)
			port = defaultPort
		} else {
			port = p
		}
...
func main() {
	...
	fmt.Println("=== gRPC audio transcriber client ===")
	host, port := getServerAddressFromUser(grpcCfg.Listener.Address, grpcCfg.Listener.Port)
